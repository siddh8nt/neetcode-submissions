        ...
        while (i < nums.length - 2) {
            ...
            while (j < k) {
                if (nums[j] + nums[k] > target) {
                    ...
                    while (j < nums.length
                        && nums[j - 1] == nums[j]) { // 1st && condition to avoid array index out of
                                                     // bounds error
                        j++;
                    }
                    while (i > 0 && nums[k + 1] == nums[k]) { // 1st && condition to avoid array
            ...
            while (i < nums.length - 2
                && nums[i]
                    == nums[i - 1]) { // 1st && condition to avoid array index out of bounds error
                i++;
            }
        }
        return result;
